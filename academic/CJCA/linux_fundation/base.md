...
## 常用日志文件
- Kernel logs：日志包含有关系统内核的信息，包括硬件驱动程序、系统调用和内核事件。它们存储在 /var/log/kern.log 文件中。例如，内核日志可以显示存在易受攻击或过时的驱动程序，攻击者可能利用这些驱动程序获取系统访问权限。它们还可以提供有关系统崩溃、资源限制及其他可能导致拒绝服务或其他安全问题的事件的见解。此外，内核日志可以帮助我们识别可疑的系统调用或其他活动，这可能表明系统上存在恶意软件或其他恶意程序。通过监控 /var/log/kern.log 文件，我们可以检测任何异常行为，并采取适当的措施防止系统进一步受损。
  
- System logs：包含有关系统级事件的信息，例如服务启动和停止、登录尝试以及系统重启。它们存储在 /var/log/syslog 文件中。通过分析登录尝试、服务的启动和停止以及其他系统级事件，我们可以检测系统上可能发生的任何访问或活动。这有助于我们识别可能被利用的漏洞，并帮助我们提出安全措施以减轻这些风险。此外，我们还可以使用 syslog 来识别可能影响系统可用性或性能的潜在问题，例如服务启动失败或系统重启。
- Authentication logs：包含有关用户身份验证尝试的信息，包括成功和失败的尝试。它们存储在 /var/log/auth.log 文件中。需要注意的是，尽管 /var/log/syslog 文件可能包含类似的登录信息，但 /var/log/auth.log 文件专门关注用户身份验证尝试，使其成为识别潜在安全威胁的更有价值的资源。
- Application logs: 在var/log下，包含有关系统上运行的特定应用程序活动的信息。它们通常存储在各自的文件中，例如 Apache 网络服务器的 /var/log/apache2/error.log 或 MySQL 数据库服务器的 /var/log/mysql/error.log。
- Access Log Entry访问日志access.log：也是应用下的，例如 Apache 网络服务器的 /var/log/apache2/access.log，MySQL 数据库服务器的/var/log/mysql/mysql.log，systemd的/var/log/journal/ 
- Security logs安全日志: 安全日志及其事件通常会记录在各种日志文件中，具体取决于所使用的安全应用或工具。例如，Fail2ban 应用会将失败的登录尝试记录在 /var/log/fail2ban.log 文件中，而 UFW 防火墙会将活动记录在 /var/log/ufw.log 文件中。其他与安全相关的事件，例如系统文件或设置的更改，可能会记录在更通用的系统日志中，如 /var/log/syslog 或 /var/log/auth.log。